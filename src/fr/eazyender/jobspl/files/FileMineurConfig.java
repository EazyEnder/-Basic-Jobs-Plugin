...
public class FileMineurConfig {
	
	public static FileMineurConfig fileMineurConfig;
	...
	private static File MineurjobFile;
	private static FileConfiguration MineurStatsConfig;
	
	public FileMineurConfig() {
			fileMineurConfig = this;
		    registerFile();
		    loadValues();
	  }
	
	private void registerFile() {
		MineurjobFile = new File(JobsPlMain.getJobsPl().getDataFolder(), "ConfigMineur.yml");
		MineurStatsConfig = YamlConfiguration.loadConfiguration(MineurjobFile);
		    saveFile();
		  }
	
	public static void createBasicFile() {
		for(int i = 1; i <= 50; i++) {
	    String baseStringValue = "broadcast le joueur %player% est niveau " + i + " MINEUR";
	    ...
	    String str = i + "";
	    ConfigurationSection s = MineurStatsConfig.createSection(str);  
	...
	private static void saveFile() {
	    try {		    
	    	MineurStatsConfig.save(MineurjobFile);
	    } catch (IOException iOException) {}
	  }
	
	public void loadValues() {
	    if (MineurStatsConfig.contains("1")) {
	      for(int i = 1; i <= 50; i++) {
	    	  String str = i + "";
	    	ConfigurationSection s = MineurStatsConfig.getConfigurationSection(str);
