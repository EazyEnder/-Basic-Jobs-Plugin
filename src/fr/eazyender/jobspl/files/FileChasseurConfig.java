...
public class FileChasseurConfig {
	
	public static FileChasseurConfig filechasseurConfig;
	...
	private static File ChasseurjobFile;
	private static FileConfiguration ChasseurStatsConfig;
	
	public FileChasseurConfig() {
		filechasseurConfig = this;
		    registerFile();
		    loadValues();
	  }
	
	private void registerFile() {
		ChasseurjobFile = new File(JobsPlMain.getJobsPl().getDataFolder(), "ConfigChasseur.yml");
		ChasseurStatsConfig = YamlConfiguration.loadConfiguration(ChasseurjobFile);
		    saveFile();
		  }
	
	public static void createBasicFile() {
		for(int i = 1; i <= 50; i++) {
	    String baseStringValue = "broadcast le joueur %player% est niveau " + i + " CHASSEUR";
	    ...
	    String str = i + "";
	    ConfigurationSection s = ChasseurStatsConfig.createSection(str);  
	...
	private static void saveFile() {
	    try {		    
	    	ChasseurStatsConfig.save(ChasseurjobFile);
	    } catch (IOException iOException) {}
	  }
	
	public void loadValues() {
	    if (ChasseurStatsConfig.contains("1")) {
	      for(int i = 1; i <= 50; i++) {
	    	  String str = i + "";
	    	ConfigurationSection s = ChasseurStatsConfig.getConfigurationSection(str);
